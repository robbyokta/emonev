...
class mtr_pengadaan extends CI_Controller {
	...
	public function index()
	{
		if($this->session->userdata('isLogin') == FALSE) {
                ...
                //setting view
                $data = array();
                $data['title'] = 'Master Data Pengadaan <small> </small>';
                $data['pengguna'] = $user;
                ...
                $this->load->view('mtr_pengadaan/index', $data);
                }
	}

    public function data_mtr_pengadaan(){
        $this->load->library( 'datatables');
        $user = $this->session->userdata('data_user');
        $role = $user->role;
        if ($role == "1"){
            $this->datatables->select('skpd.nama_skpd as skpd,kode, nama_pengadaan, anggaran, id_pengadaan');
     ...
     public function detailpengadaan($id){
        if($this->session->userdata('isLogin') == FALSE) {
                ...
                $this->db->select('mtr_pengadaan.*, skpd.nama_skpd');
                $this->db->from('mtr_pengadaan');
                $this->db->join('skpd', 'mtr_pengadaan.id_skpd = skpd.id_skpd');
                $this->db->where('mtr_pengadaan.id_pengadaan', $id);
                $query = $this->db->get();
                $data['data'] = $query->row();
                $this->db->select('*');
               $this->db->from('dt_tgt_pengadaan');
               $this->db->where('dt_tgt_pengadaan.id_pengadaan', $id);
               $query = $this->db->get();
                $data['target'] = $query->result();

                
                $this->load->view('mtr_pengadaan/detailpengadaan', $data);
            }
    ...
    public function simpandetail(){
        ...
        $data = array(
               ...
               'pemilihan_penyedia_awal' => $pemilihan_penyedia_awal,
               'pemilihan_penyedia_akhir' => $pemilihan_penyedia_akhir,
               'pelaksana_pekerjaan_awal' => $pelaksana_pekerjaan_awal,
               'pelaksana_pekerjaan_akhir' => $pelaksana_pekerjaan_akhir,
            );
        $this->db->where('id_pengadaan', $id_pengadaan);
        $this->db->update('mtr_pengadaan', $data); 
        $query = $this->db->query("SELECT * FROM (`mtr_pengadaan`) WHERE `mtr_pengadaan`.`id_pengadaan` = '$id_pengadaan'");
        $detail = $query->result();
     ...
     public function tambahpengadaan(){
        ...
        $data = array(
               ...
               'jenis_belanja' => $this->input->post('jenis_blj'),
        ...
        $this->db->insert('mtr_pengadaan', $data); 
    ...
    public function simpantarget(){
        ...
        $this->db->update('dt_tgt_pengadaan', $data[$i]);  
        }  

        $query = $this->db->query("SELECT * FROM (`dt_tgt_pengadaan`) WHERE `id_pengadaan` = '$id_pengadaan'");
        $detail = $query->result();
